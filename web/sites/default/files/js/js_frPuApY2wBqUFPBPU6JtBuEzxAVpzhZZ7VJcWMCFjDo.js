...
document.querySelector('.hamburgericon').addEventListener('click', () => {
    if (!flag) {
        ...
        document.querySelector('.menu-toggle').classList.add('hb').setAttribute("style", "display:flex;");
        flag = true;
    } else {
       document.querySelector('.menu-toggle').classList.remove('menu-untoggle');   // Toggle/click to close.
        document.querySelector('.menu-toggle').classList.remove('hb').setAttribute("style", "");
        flag = false;
    }
});
